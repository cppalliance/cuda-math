...
#include <boost/math/special_functions/hypergeometric_pFq.hpp>
extern "C" __global__ 
void test_hypergeometric_pFq_kernel(const float_type *in1, const float_type* in2, float_type *out, int numElements)
{
...
int main() 
{
    try
    {
        ...
        res = nvrtcCreateProgram(&prog, cuda_kernel, "test_hypergeometric_pFq_kernel.cu", 0, nullptr, nullptr);
        checkNVRTCError(res, "Failed to create NVRTC program");

        nvrtcAddNameExpression(prog, "test_hypergeometric_pFq_kernel");
        ...
        checkCUError(cuModuleGetFunction(&kernel, module, "test_hypergeometric_pFq_kernel"), "Failed to get kernel function");
        ...
        for (int i = 0; i < numElements; ++i) 
        {
            const auto res = boost::math::hypergeometric_pFq(std::initializer_list<float_type>({h_in1[i]}), std::initializer_list<float_type>({h_in2[i]}), static_cast<float_type>(1));
