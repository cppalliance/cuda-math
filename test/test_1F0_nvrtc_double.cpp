...
#include <boost/math/special_functions/hypergeometric_1F0.hpp>
extern "C" __global__ 
void test_hypergeometric_1F0_kernel(const float_type *in1, const float_type* in2, float_type *out, int numElements)
{
    int i = blockDim.x * blockIdx.x + threadIdx.x;
    if (i < numElements)
    {
        out[i] = boost::math::hypergeometric_1F0(in1[i], in2[i]);
    }
}
)";
...
int main() 
{
    try
    {
        ...
        res = nvrtcCreateProgram(&prog, cuda_kernel, "test_hypergeometric_1F0_kernel.cu", 0, nullptr, nullptr);
        checkNVRTCError(res, "Failed to create NVRTC program");

        nvrtcAddNameExpression(prog, "test_hypergeometric_1F0_kernel");
        ...
        checkCUError(cuModuleGetFunction(&kernel, module, "test_hypergeometric_1F0_kernel"), "Failed to get kernel function");
        ...
        for (int i = 0; i < numElements; ++i) 
        {
            const auto res = boost::math::hypergeometric_1F0(h_in1[i], h_in2[i]);
