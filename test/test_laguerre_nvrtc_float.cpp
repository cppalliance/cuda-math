...
#include <boost/math/special_functions/laguerre.hpp>
extern "C" __global__ 
void test_laguerre_kernel(const float_type *in1, const float_type* in2, float_type *out, int numElements)
{
    int i = blockDim.x * blockIdx.x + threadIdx.x;
    if (i < numElements)
    {
        out[i] = boost::math::laguerre(1, in1[i], in2[i]);
    }
}
)";
...
int main() 
{
    try
    {
        ...
        res = nvrtcCreateProgram(&prog, cuda_kernel, "test_laguerre_kernel.cu", 0, nullptr, nullptr);
        checkNVRTCError(res, "Failed to create NVRTC program");

        nvrtcAddNameExpression(prog, "test_laguerre_kernel");
        ...
        checkCUError(cuModuleGetFunction(&kernel, module, "test_laguerre_kernel"), "Failed to get kernel function");
        ...
        for (int i = 0; i < numElements; ++i) 
        {
            const auto res = boost::math::laguerre(1, h_in1[i], h_in2[i]);
