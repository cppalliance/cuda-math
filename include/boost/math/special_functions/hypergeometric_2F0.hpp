...
namespace boost { namespace math { namespace detail {
   ...
   struct hypergeometric_2F0_cf
   {
      ...
      BOOST_MATH_GPU_ENABLED hypergeometric_2F0_cf(T a1_, T a2_, T z_) : a1(a1_), a2(a2_), z(z_), k(-2) {}
      ...
      BOOST_MATH_GPU_ENABLED result_type operator()()
      {
         ...
         if (k <= 0)
         {
            return boost::math::make_pair(z * a1 * a2, 1);
         }
         return boost::math::make_pair(-z * (a1 + k) * (a2 + k) / (k + 1), 1 + z * (a1 + k) * (a2 + k) / (k + 1));
      }
   };

   template <class T, class Policy>
   BOOST_MATH_GPU_ENABLED T hypergeometric_2F0_cf_imp(T a1, T a2, T z, const Policy& pol, const char* function)
   {
   ...
   template <class T, class Policy>
   BOOST_MATH_GPU_ENABLED inline T hypergeometric_2F0_imp(T a1, T a2, const T& z, const Policy& pol, bool asymptotic = false)
   {
      //
      ...
      constexpr auto function = "boost::math::hypergeometric_2F0<%1%,%1%,%1%>(%1%,%1%,%1%)";
      ...
      {
         BOOST_MATH_GPU_SAFE_SWAP(a1, a2);
         BOOST_MATH_GPU_SAFE_SWAP(is_a1_integer, is_a2_integer);
      }
      //
      ...
      if ((a1 == a2 - 0.5f) && (z < 0))
      {
         ...
         int n = static_cast<int>(static_cast<boost::math::uintmax_t>(boost::math::lltrunc(-2 * a1)));
         T smz = sqrt(-z);
      ...
      if (is_a1_integer && is_a2_integer)
      {
         if ((a1 < 1) && (a2 <= a1))
         {
            const unsigned int n = static_cast<unsigned int>(static_cast<boost::math::uintmax_t>(boost::math::lltrunc(-a1)));
            const unsigned int m = static_cast<unsigned int>(static_cast<boost::math::uintmax_t>(boost::math::lltrunc(-a2 - n)));
         ...
         else if ((a2 < 1) && (a1 <= a2))
         {
            ...
            const unsigned int n = static_cast<unsigned int>(static_cast<boost::math::uintmax_t>(boost::math::lltrunc(-a2)));
            const unsigned int m = static_cast<unsigned int>(static_cast<boost::math::uintmax_t>(boost::math::lltrunc(-a1 - n)));
...
BOOST_MATH_GPU_ENABLED inline typename tools::promote_args<T1, T2, T3>::type hypergeometric_2F0(T1 a1, T2 a2, T3 z, const Policy& /* pol */)
{
...
BOOST_MATH_GPU_ENABLED inline typename tools::promote_args<T1, T2, T3>::type hypergeometric_2F0(T1 a1, T2 a2, T3 z)
{
